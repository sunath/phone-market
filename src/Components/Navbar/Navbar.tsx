...
const Navbar = () => {
  ...
  const currentCount = useSelector((store:any) => {
    let count = 0;
    if(!store.cart || !store.cart.items)return 0;
      return Object.keys(store.cart.items).map(e => {
        const count =  Object.keys(store.cart.items[e]).map(x => store.cart.items[e][x].items).reduce((p,n) => p+n);
        return count
      }).reduce((p,n) => p+n)
  })
  ...
  <MenuItem onClick={createNavigationAction('/shopping-cart')}>
